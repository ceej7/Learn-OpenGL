...
void ConfigureShaderAndMatrices(glm::mat4 _view, glm::mat4 _projection);
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		ConfigureShaderAndMatrices(lightView,lightProjection);
		...
		ConfigureShaderAndMatrices(camera.GetViewMatrix(),
			glm::perspective(glm::radians(camera.Zoom), (float)screenWidth / screenHeight, 0.1f, 1000.0f));
...
void RenderScene(Shader& shader)
{
	...
	model = glm::rotate(model, glm::radians(30.0f*(float)glfwGetTime()+50.0f), glm::normalize(glm::vec3(0.0,1.0, 1.0)));
	model = glm::scale(model, glm::vec3(0.5f));
	shader.setMat4("model", model);
	shader.setMat4("view", view);
	shader.setMat4("projection", projection);
